...
const AttendeeScreen = ({navigation,route}) => {
  ...
  const renderLists = () => {
    ...
    const renderItem = ({ item }) => (
      <View style={tw`bg-white flex-row justify-between items-center mb-2 p-2 
       rounded-lg border-b border-gray-300`}>
      {/**Content */}
      <View style={tw`flex-row items-center w-8/12`}>
      <Icon type='feather' name={item.status == 'checked' ? 'check-circle' : 'alert-octagon'} size={20} color='white'
      containerStyle={tw`${item.status == 'checked'? 'bg-green-400': 'bg-yellow-400'} rounded p-3`}/>

        <View style={tw`p-3 `}>
        <Text style={tw`text-sm text-gray-700 font-bold`}>ID:{item.reference}</Text>
        <Text style={tw`text-base text-gray-600 font-semibold`} numberOfLines={1}>{item.fullname}</Text>
        <View style={tw`flex-row`}>
              <Text style={tw`text-sm font-semibold text-gray-500`}>{item.ticket.title}</Text>
              <Text style={tw`text-sm font-semibold text-gray-500 mx-1`}>|</Text>
              <Text style={tw`text-sm font-semibold text-gray-500`}>GHS{item.ticket.price}</Text>
              </View>
        </View>
      </View>
        {/**Time */}
        <View style={tw`p-3 items-center`}>
        <Text style={tw`text-xs text-gray-400 font-medium`}>{item.checkIn_time}</Text>
        <TextButton 
        label='Check-in'labelStyle={tw`text-sm`}
