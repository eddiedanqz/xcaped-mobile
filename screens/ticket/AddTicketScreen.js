...
const AddTicketScreen = ({ navigation,route}) => {
  ...
  const refInputs = useRef([{title:'',price:'',capacity:''}]);
    ...
    for (let i = 0; i < numInputs; i++) {
      form.push(
        ...
        <TouchableOpacity style={[tw`items-center justify-center`,{}]} onPress={() => removeForm(i)}>
    ...
    const handleSubmit = () => {
      setTickets(refInputs.current)
        ...
        axios.post(`${BASEURL}/api/ticket`,formData,config)
            .then((res) => {
  ...
  return (
    <SafeAreaView style={tw`h-full bg-white`}>
      ...
      <View
        style={[tw`flex-row w-full h-16 items-center justify-center px-3 shadow `,
        {backgroundColor: '#fff',
          shadowColor: '#000',
      ...
      <Section containerStyle={tw`items-center justify-center mb-1`}>
            <TextButton label='Finish'
              buttonContainerStyle={[tw`rounded-lg p-3 w-72`,{backgroundColor:'#ff8552'}]}
