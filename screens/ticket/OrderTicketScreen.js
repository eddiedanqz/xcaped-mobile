...
const OrderTicketScreen = ({ navigation, route }) => {
   ...
   function sumOrder() {
    ...
    axios.post(`${BASEURL}/api/order`,formData,config)
        .then((res) => {
    ...
    const renderTickets = () => {
        const renderItem = ({ item }) => (
            <View style={tw`flex-row bg-white w-full shadow rounded-lg h-28 my-3`}>
              <View style={[tw`bg-gray-100 absolute rounded-full p-2.5`,{left:132.3,top:-12}]}></View>
              <View style={[tw`bg-gray-100 absolute rounded-full p-2.5`,{left:132.3,bottom:-12}]}></View>
            {/**left */}
            <View style={[tw`justify-center items-center w-5/12 border-r border-gray-100`,]}>
            <Text style={tw`text-base text-gray-600 `}>{item.title}</Text>
            <Text style={[tw`text-xl text-gray-700 font-bold m-2`,{color:'#ff8552'}]}>${item.price}</Text>
            <Text style={tw`text-sm text-gray-500`}>Per person</Text>
            </View>
            {/**right */}
     ...
     {/**Tickets */}
      ...
      <View style={tw`flex-row bg-white w-full bottom-0`}>
           ...
           <TextButton label='Buy'
            buttonContainerStyle={[tw`rounded-lg p-3 w-48`,{backgroundColor:'#ff8552'}]}
               onPress={() => order()}
             />
