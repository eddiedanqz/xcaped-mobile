...
const MapModal = ({ isVisible, onClose,ticketId}) => {
...
const [modalVisible, setModalVisible] = useState(false);
  ...
  function renderSearch() {
    return (
       <View style={tw`flex-row items-center justify-center bg-gray-200 mx-4 my-2 py-1.5 px-3 rounded-lg`} >
        <TextInput  style={tw`flex-1 text-lg`} 
         placeholder='Search...'
 ...
 const renderUsers = () => {
    ...
    const renderItem = ({ item }) => (
      <TouchableOpacity style={tw`flex-row items-center justify-center mb-2 p-1`} 
      onPress={() => {
        setId(item.id)
        ...
        <View style={tw`flex-row items-center bg-white`}
		 ...
		 >
			<View
          style={tw`bg-white w-16 h-16 shadow-xl`}
        >

          <Image
            source={item.profile?.banner ? {uri:`${BASEURL}/storage/images/user/${item.profile?.banner}` } : noImage}
            resizeMode="stretch"
            style={tw`w-full h-full rounded p-1`}
          />
		</View>
          <View style={tw`flex-1 justify-start items-start px-5`}>
            <Text style={tw`text-lg text-gray-700 font-bold`}> {item.fullname} </Text>
        ...
        {/**Check button */}
          ...
          <View
              style={tw`rounded-full p-1 mx-1 border-2 ${item.id === id ? 'border-green-400' : "border-gray-400"}
            `}
              onPress={() => selectHandler(item)}
            >
                  <Icon name="check" type="feather" size={12}
                   color={`${item.id === id ? 'white' : "gray"}`}
                  containerStyle={tw`rounded-full  ${item.id === id ? 'bg-green-400' : ""}`}/>
            </View>
        </View>
      </TouchableOpacity>
    );
      ...
      >
        ...
        <View style={tw`flex-1 justify-center items-center absolute inset-0`}>
          <View style={tw`bg-white rounded p-6`}>
            <Text style={tw`font-bold text-center text-base text-gray-700`}>Are you sure?</Text>
            <Text style={tw`text-center mb-5 mt-1 mx-2 text-gray-600`}>This action cannot be reversed.</Text>
            <View style={tw`flex-row justify-between items-center`}>
            <TextButton label='Cancel' buttonContainerStyle={tw`p-1 bg-transparent`} labelStyle={tw`text-red-400`}
             onPress={() => setModalVisible(false)}/>
            <TextButton label='Yes' buttonContainerStyle={tw`p-1 bg-transparent`} labelStyle={tw`text-black`}
             onPress={() => {
                send();
