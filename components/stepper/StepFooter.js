...
const StepFooter = ({ navigation , steps,currentStep,setCurrentStep,params,inputs}) => {
...
SecureStore.getItemAsync("mytoken").then((token) => {
  let parsed = JSON.parse(token)
const config = {
  headers: { Authorization: `Bearer ${parsed}`,"Content-Type": "multipart/form-data",}
}
  axios.post(`${BASEURL}/api/event/create`,params,config)
    .then((res) => {
      if(res.data){
  ...
  return (
    <View style={tw`flex-1 justify-end`}>
      {isError && 
      (<View style={tw`flex flex-row justify-between items-center bg-red-600 z-10 mx-1 p-2 bottom-20 rounded z-10`}>
    ...
    <View style={tw`w-1/2 border-r items-center justify-center`}>
      <TouchableOpacity style={tw`justify-center items-center`} onPress={() => {
        if(currentStep > 0){
    ...
    {(currentStep+1) < steps.length /* add other conditions here */ &&
     <View style={tw`w-1/2 items-center justify-center`}>
      <TouchableOpacity style={[tw`justify-center items-center`,]} onPress={() => {
        ...
        <Text style={[tw`text-base text-black`/*,{color:'#fdcc97'}*/]}>Next</Text>
      </TouchableOpacity>
      </View>
    }
    {(currentStep+1) == steps.length /* add other conditions here */ &&
     <View style={tw`w-1/2 items-center justify-center`}>
      <TouchableOpacity style={tw`justify-center items-center`} onPress={checkFields}>
        <Text style={[tw`text-base text-black`/*,{color:'#fdcc97'}*/]}>Finish</Text>
      </TouchableOpacity>
      </View>
    }
  </View>
