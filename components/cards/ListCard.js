...
const ListCard = ({ containerStyle, item, onPress, iconName,iconMethod,isVisible ,activeItem}) => {
  // const [showModal, setShowModal] = useState(isVisible);
  const [id, setId] = useState(null);

  return (
    <View style={[tw`flex-row justify-between items-center mb-2 p-1`,containerStyle]}>
      ...
      >
        ...
        <View style={tw`px-1`}>
          {/**Title */}
          <Text style={tw`text-base font-bold`} numberOfLines={1}> {item.title} </Text>
          {/**Location */}
                  ...
                  {item.ticket?.map((ticket, i) => {
                    return (
                      <Text
                        key={`${ticket.id}`}
                        style={[
                          tw`text-lg text-gray-600 font-bold text-black`,
                        ]}
                      >
                        ${ticket.price}
